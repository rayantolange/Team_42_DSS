...
const STATUS_CONFIG: Record<
  string,
  {
    label: string;
    dot: string;
    badge: "soft-success" | "soft-destructive" | "soft" | "soft-warning";
  }
> = {
    ...
    <Card>
      <CardHeader className="flex-row items-center justify-between space-y-0">
        <div>
          ...
          <CardDescription>
            Latest institutional decisions across departments
          </CardDescription>
        </div>
        <Link
          to="/history"
          ...
          <>
            ...
            <div className="divide-y divide-border sm:hidden">
              {decisions.map((decision, i) => {
                const status =
                  STATUS_CONFIG[decision.status] ?? STATUS_CONFIG.draft!;
                return (
                  <Link
                    key={decision.decisionId}
                    to={`/decisions/${decision.decisionId}`}
                    className="flex animate-fade-in items-start justify-between gap-3 px-4 py-3 transition-colors hover:bg-muted/40"
                    style={{
