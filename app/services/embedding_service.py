...
class EmbeddingService:
    ...
    def embed_document_chunks_batch(
        ...
        """
        Batched version of embed_document_chunk — encodes all of a page's
        chunks in one model call instead of one call per chunk.
        """
        from app.ai.embedding_client import embed_documents_batch

        document = page.document
        decision = document.decision if document else None
        department_id = decision.department_id if decision else None

        vectors = embed_documents_batch(chunk_texts)
